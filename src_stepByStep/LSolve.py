...
initGridFile = filedialog.askopenfilename() #gets input file

Tk().withdraw()#gets output directory
messagebox.showinfo("LSolve", "Please select an output directory")
directory = filedialog.askdirectory()

#checks if file type is correct
if not initGridFile.endswith('.tsv') and not initGridFile.endswith('.csv') and not initGridFile.endswith('.txt'):
...
os.chdir(directory)
initGrid = np.loadtxt(initGridFile, dtype=float)#this reduces complexity of input tsv files. One no longer has to put the number of rows and columns in the first line of the file
rows = initGrid.shape[0]
columns = initGrid.shape[1]

commandString = ["\"",laplaceExecutable,"\" \"",initGridFile,"\" ",str(rows)," ",str(columns)]
